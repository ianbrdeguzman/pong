...
const scoreControl = () => {
    ...
    if (player.score >= 3 || enemy.score >= 3) {
        ...
        // if player score is greater
        if (player.score > enemy.score) {

            // show winner screen
            winnerScreen.classList.toggle('retry');

        // if enemy score is greater
        } else {
...
const gameLoop = () => {
    ...
    if(collisionControl(ball, paddle)) {
        ...
        ball.dx = direction * (ball.speed * 2) * Math.cos(angle);
        ball.dy = (ball.speed * 2) * Math.sin(angle);
    }
